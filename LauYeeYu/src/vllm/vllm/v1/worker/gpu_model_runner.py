...
class GPUModelRunner(
    ...
    def _prepare_inputs(
        ...
        # Override positions for one-step hole-filling requests.
        # These have non-contiguous gap positions that can't be expressed
        # as num_computed_tokens + arange.
        hole_fill_gap_ranges = scheduler_output.hole_fill_gap_ranges
        if hole_fill_gap_ranges:
            for req_id, gap_ranges in hole_fill_gap_ranges.items():
                req_index = self.input_batch.req_id_to_index[req_id]
                # Find the start offset for this request in the flat array.
                start = int(cu_num_tokens[req_index - 1]) if req_index > 0 \
                    else 0
                offset = 0
                for gap_start, gap_len in gap_ranges:
                    positions_np[start + offset:start + offset + gap_len] = (
                        np.arange(gap_start, gap_start + gap_len,
                                  dtype=np.int32))
                    offset += gap_len
        ...
        # Override seq_lens for hole-filling requests.
        # seq_lens = end of last gap (all blocks up to there are allocated).
        if hole_fill_gap_ranges:
            for req_id, gap_ranges in hole_fill_gap_ranges.items():
                req_index = self.input_batch.req_id_to_index[req_id]
                last_gap_start, last_gap_len = gap_ranges[-1]
                self.seq_lens.np[req_index] = last_gap_start + last_gap_len
    ...
    def _build_attention_metadata(
        ...
        hole_fill_gap_ranges: dict[str, list[tuple[int, int]]] | None = None,
        ...
        # Convert req_id-keyed hole fill ranges to batch-index-keyed.
        if hole_fill_gap_ranges:
            idx_gap_ranges: dict[int, list[tuple[int, int]]] = {}
            for req_id, gap_ranges in hole_fill_gap_ranges.items():
                if req_id in self.input_batch.req_id_to_index:
                    req_idx = self.input_batch.req_id_to_index[req_id]
                    idx_gap_ranges[req_idx] = gap_ranges
            if idx_gap_ranges:
                cm_base.hole_fill_gap_ranges = idx_gap_ranges
        ...
        with (
            ...
            attn_metadata, spec_decode_common_attn_metadata = (
                self._build_attention_metadata(
                    ...
                    hole_fill_gap_ranges=(
                        scheduler_output.hole_fill_gap_ranges or None),
                )
            )
