...
class BlockHashToBlockMap:
    ...
    from the inner dict.
    NOTE #3: When multiple blocks have the same hash, we maintain a
    "canonical" block concept - the block that would be returned by
    get_one_block() (the first block in iteration order). When blocks
    are freed, non-canonical blocks are evicted immediately to prevent
    duplicate blocks from accumulating in the radix tree. This ensures
    that the radix tree-based eviction policies work correctly without
    needing to handle duplicates.
    """
    ...
    def is_canonical_block(
        self, key: BlockHashWithGroupId, block_id: int
    ) -> bool:
        """
        Check if the given block_id is the canonical block for the given key.
        The canonical block is the one that would be returned by get_one_block.

        Args:
            key: The block hash key.
            block_id: The block ID to check.

        Returns:
            True if this block_id is the canonical block, False otherwise.
        """
        blocks = self._cache.get(key)
        if blocks is None:
            return False
        ...
        if isinstance(blocks, dict):
            # For dict, the canonical block is the first one in iteration order
            # which matches what get_one_block returns
            canonical_block = next(iter(blocks.values()))
            return canonical_block.block_id == block_id
        self._unexpected_blocks_type(blocks)
        return False
...
class BlockPool:
    ...
    def __init__(
        ...
        # enabled).
        import vllm.v1.core.free_block_manager as fbm
        block_manager_cls = getattr(fbm, f"{block_manager_algo}FreeBlockManager")
        self.free_block_manager = block_manager_cls(self.blocks)
        ...
        # avoid freeing it.
        self.null_block = self.free_block_manager.get_free_blocks_n(1)[0]
    ...
    def get_new_blocks(self, num_blocks: int) -> list[KVCacheBlock]:
        ...
        ret: list[KVCacheBlock] = self.free_block_manager.get_free_blocks_n(num_blocks)
    ...
    def touch(self, blocks: Sequence[KVCacheBlock]) -> None:
        ...
        for block in blocks:
            ...
            if block.ref_cnt == 0 and not block.is_null:
                self.free_block_manager.remove(block)
            block.ref_cnt += 1
            if self.metrics_collector:
                self.metrics_collector.on_block_accessed(block)

    def free_blocks(
        self, ordered_blocks: Iterable[KVCacheBlock], record_request: bool = False,
    ) -> None:
        """Free a list of blocks. The blocks should be ordered by their
        eviction priority, where the first block will be evicted first.
        We free the blocks in reverse order so that the tail blocks are evicted
        first for algorithms like LRU.

        When freeing blocks with hashes, we check if they are the canonical
        (major) block for that hash. Non-canonical blocks are evicted
        immediately to prevent duplicate blocks with the same hash from
        accumulating in the radix tree.

        Args:
            ordered_blocks: A list of blocks to free ordered by their eviction
                priority.
            record_request: Whether to record the request blocks in the free
                block manager.
        """
        ...
        if record_request:
            self.free_block_manager.record_request_blocks(blocks_list)

        # Second pass: handle non-canonical blocks with hashes
        # If a block has a hash but is not the canonical block for that hash,
        # evict it immediately to prevent duplicates in the radix tree
        if self.enable_caching:
            for block in blocks_list:
                if block.is_null:
                    continue
                if block.block_hash is not None and block.ref_cnt == 0:
                    # Check if this block is the canonical one for its hash
                    if not self.cached_block_hash_to_block.is_canonical_block(
                        block.block_hash, block.block_id
                    ):
                        # This is a non-canonical duplicate block
                        # Evict it immediately
                        self._maybe_evict_cached_block(block)

        # Third pass: add blocks with ref_cnt == 0 to free list
        reversed_blocks_list = reversed(blocks_list)
        self.free_block_manager.add_n(
            [block for block in reversed_blocks_list if block.ref_cnt == 0 and not block.is_null]
        )
    ...
    def get_num_free_blocks(self) -> int:
        ...
        """
        return self.free_block_manager.num_free_blocks
