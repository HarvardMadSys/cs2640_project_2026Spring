...
- `--dataset synthetic|taxi`
- `--input <path>` for Taxi Parquet input
...
Synthetic workloads are generated by the benchmark itself, so no external data is needed for the default run. For the Taxi workload, pass a local Parquet path with `--dataset taxi --input <path>`.
