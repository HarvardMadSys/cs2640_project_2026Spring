...
from typing import Any, Literal, NewType, TypeAlias, overload
...
@dataclass
class KVCacheBlockSegment:
    """KV-cache block segment metadata."""

    blocks: KVCacheBlocks
    start_block_index: int
    length_in_blocks: int
...
class RadixTreeNode:
    ...
    Note: we do not record the number of evicted blocks here as this might be
    inconsistent.
    """
    # The block_hashes cannot be KVCacheBlock, because it might not exist in
    # the block pool
    block_hashes: list[BlockHashWithGroupId]  # Only root can be empty
    in_use: list[bool] = field(default_factory=list)
    parent: Optional['RadixTreeNode'] = None
    children: dict[BlockHashWithGroupId, 'RadixTreeNode'] = field(default_factory=dict)
    ...
    def init(
        ...
        default_in_use: bool = True,
    ) -> None:
        """Initialize the radix tree node.

        Args:
            block_hashes: The list of block hashes.
            default_in_use: The default value of in_use. (Usually True for
            because the registration is finished before putting free blocks
            into the free block manager)
        """
        self.block_hashes = block_hashes
        self.in_use = [default_in_use] * len(block_hashes)
        self.parent = parent
        children_map = {}
        for child in children:
            children_map[child.block_hashes[0]] = child
        self.children = children_map


class RadixTree:
    """
    A radix tree for storing block hashes.

    To make the structure easier to maintain, we prohibit the existence of chain
    of nodes that have only one child except for the root node.
    This ensures the eviction on the leaf nodes do not need recursive calls to evict
    their parent nodes.
    """
    def __init__(self):
        self._root = RadixTreeNode(block_hashes=[], parent=None, children={})
        self._node_map: dict[BlockHashWithGroupId, tuple[RadixTreeNode, int]] = {}
        self._free_nodes: set[RadixTreeNode] = set()
        self._in_use_nodes: set[RadixTreeNode] = set()
        self._leaf_nodes: set[RadixTreeNode] = set()

    def _update_node_lists(self, node: RadixTreeNode) -> None:
        """Update the _free_nodes and _in_use_nodes sets for a given node."""
        if node == self._root:
            return
        
        has_in_use = any(node.in_use)
        if has_in_use:
            self._free_nodes.discard(node)
            self._in_use_nodes.add(node)
        else:
            self._free_nodes.add(node)
            self._in_use_nodes.discard(node)

    def add_sequence(self, block_hashes: list[BlockHashWithGroupId]) -> None:
        if not block_hashes:
            return
        ...
        while seq_idx < len(block_hashes):
            hash_val = block_hashes[seq_idx]
            if hash_val in curr.children:
                child = curr.children[hash_val]
                
                # Find the longest common prefix
                match_len = 0
                while (match_len < len(child.block_hashes) and 
                       seq_idx + match_len < len(block_hashes) and 
                       child.block_hashes[match_len] ==
                       block_hashes[seq_idx + match_len]):
                    match_len += 1
                
                if match_len == len(child.block_hashes):
                    # Full match with the child node's blocks
                    curr = child
                    seq_idx += match_len
                elif seq_idx + match_len == len(block_hashes):
                    # The sequence is fully matched as a prefix of child.
                    # We don't need to split Because doing so would create a node with exactly one child
                    curr = child
                    seq_idx += match_len
                else:
                    # Partial match: split the child node
                    remaining_hashes = child.block_hashes[match_len:]
                    
                    split_node = RadixTreeNode(
                        block_hashes=remaining_hashes,
                        parent=child,
                    )
                    split_node.in_use = child.in_use[match_len:]
                    
                    # Update old child (which becomes the prefix)
                    child.block_hashes = child.block_hashes[:match_len]
                    child.in_use = child.in_use[:match_len]
                    ...
                    child.children = {split_node.block_hashes[0]: split_node}
                    
                    # Update node_map
                    for i, h in enumerate(split_node.block_hashes):
                        self._node_map[h] = (split_node, i)
                        
                    for i, h in enumerate(child.block_hashes):
                        self._node_map[h] = (child, i)
                        
                    self._update_node_lists(split_node)
                    self._update_node_lists(child)

                    # child now has children, no longer a leaf
                    self._leaf_nodes.discard(child)
                    # split_node is a leaf if original child had no children
                    if not split_node.children:
                        self._leaf_nodes.add(split_node)

                    curr = child
                    seq_idx += match_len
            else:
                # No matching child, create a new one
                new_hashes = block_hashes[seq_idx:]
                new_in_use = [True] * len(new_hashes)
                
                # If curr has NO children and is not root, adding a single child
                # would violate the invariant that non-root nodes cannot have exactly 1 child.
                # In this case, we extend curr directly.
                if curr != self._root and len(curr.children) == 0:
                    curr_hash_len = len(curr.block_hashes)
                    curr.block_hashes.extend(new_hashes)
                    curr.in_use.extend(new_in_use)
                    for i, h in enumerate(new_hashes):
                        self._node_map[h] = (curr, curr_hash_len + i)
                    self._update_node_lists(curr)
                else:
                    new_node = RadixTreeNode(
                        block_hashes=new_hashes,
                        parent=curr,
                    )
                    new_node.in_use = new_in_use
                    curr.children[new_hashes[0]] = new_node
                    
                    for i, h in enumerate(new_hashes):
                        self._node_map[h] = (new_node, i)

                    self._update_node_lists(new_node)
                    # new_node has no children, it is a leaf
                    self._leaf_nodes.add(new_node)
                break

    def touch(self, block_hash: BlockHashWithGroupId) -> None:
        """Mark the block hash as used."""
        if block_hash in self._node_map:
            node, idx = self._node_map[block_hash]
            
            if not node.in_use[idx]:
                node.in_use[idx] = True
                # If touched, it must be in the in_use_nodes set
                if node in self._free_nodes:
                    self._free_nodes.discard(node)
                    self._in_use_nodes.add(node)

    def free(self, block_hashes: list[BlockHashWithGroupId]) -> None:
        """Mark the block hashes as unused."""
        for block_hash in block_hashes:
            if block_hash in self._node_map:
                node, idx = self._node_map[block_hash]
                
                if node.in_use[idx]:
                    node.in_use[idx] = False
                    self._update_node_lists(node)

    def evict(self, node: RadixTreeNode) -> None:
        ...
        del parent.children[node.block_hashes[0]]
        self._free_nodes.discard(node)
        self._in_use_nodes.discard(node)
        self._leaf_nodes.discard(node)
        
        for h in node.block_hashes:
            self._node_map.pop(h, None)
            
        if len(parent.children) == 1 and parent != self._root:
            # Re-link the parent with its single child if it isn't the root
            sibling = next(iter(parent.children.values()))
            
            # Link sibling to parent's parent
            grandparent = parent.parent
            if grandparent is not None:
                sibling.parent = grandparent
                del grandparent.children[parent.block_hashes[0]]
                
                # Merge the hashes explicitly to preserve contiguous prefix
                # properties
                merged_hashes = parent.block_hashes + sibling.block_hashes
                merged_in_use = parent.in_use + sibling.in_use
                
                sibling.block_hashes = merged_hashes
                sibling.in_use = merged_in_use
                grandparent.children[merged_hashes[0]] = sibling
                
                # Rebuild node map for sibling since indices shifted by
                # len(parent.block_hashes)
                for i, h in enumerate(sibling.block_hashes):
                    self._node_map[h] = (sibling, i)
                    
                # The parent node is effectively bypassed entirely now
                self._free_nodes.discard(parent)
                self._leaf_nodes.discard(parent)
                self._in_use_nodes.discard(parent)
                self._update_node_lists(sibling)
                
    def _sample_nodes(
        self, n: int, node_set: set[RadixTreeNode],
    ) -> list[RadixTreeNode]:
        """Sample n nodes from the given node set. If there are less than
        n nodes, return all of them. If n is -1, return all of them."""
        import random
        pool = list(node_set)
    ...
    def sample_free_radix_tree_nodes(self, n: int) -> list[RadixTreeNode]:
        """Sample n free radix tree nodes. If there are less than n free
        radix tree nodes or n is -1, return all of them.
        """
        return self._sample_nodes(n, self._free_nodes)

    def sample_in_use_radix_tree_nodes(self, n: int) -> list[RadixTreeNode]:
        """Get n in-use radix tree nodes. If there are less than n in-use
        radix tree nodes or n is -1, return all of them.
        """
        return self._sample_nodes(n, self._in_use_nodes)

    def sample_free_leaf_nodes(self, n: int) -> list[RadixTreeNode]:
        """Sample n free leaf nodes (nodes with no children and all blocks
        free). If there are fewer than n, return all of them."""
        free_leaves = self._leaf_nodes & self._free_nodes
        return self._sample_nodes(n, free_leaves)

    def _check_radix_tree_sanity(self) -> None:
        """
        Check the sanity of the RadixTree structure.
        """
        # Check that root has no block hashes
        assert not self._root.block_hashes
        
        # Check parent-child links and no single-child non-root nodes
        def check_node(node: RadixTreeNode):
            assert node.in_use is not None
            assert len(node.block_hashes) == len(node.in_use)
            
            for k, v in node.children.items():
                assert v.parent == node
                assert v.block_hashes[0] == k
                
                # Non-root nodes with exactly one child should not exist
                # as they should be merged with their child.
                if v != self._root:
                    if len(v.children) == 1:
                        print(f"FAILED SANITY CHECK: node block_hashes={v.block_hashes} has exactly 1 child: " + str(next(iter(v.children.values())).block_hashes))
                        assert False
        ...
        # Check node sets consistency
        # All nodes in node_map should be reachable from root
        all_reachable_nodes = set()
        def get_reachable_nodes(node: RadixTreeNode):
            all_reachable_nodes.add(node)
            for child in node.children.values():
                get_reachable_nodes(child)
        get_reachable_nodes(self._root)
        
        for h, (node, _) in self._node_map.items():
            assert node in all_reachable_nodes
            assert h in node.block_hashes
            
        for node in self._free_nodes:
            assert node in all_reachable_nodes
            assert not any(node.in_use)
            
        for node in self._in_use_nodes:
            assert node in all_reachable_nodes
            assert any(node.in_use)
            
        # Check that free and in-use nodes sets are disjoint
        assert self._free_nodes.isdisjoint(self._in_use_nodes)

        # Check leaf nodes
        for node in self._leaf_nodes:
            assert node in all_reachable_nodes, \
                f"Leaf node not reachable from root"
            assert not node.children, \
                f"Leaf node has children: {len(node.children)}"
            assert node != self._root, "Root should not be in _leaf_nodes"

        for node in all_reachable_nodes:
            if node != self._root and not node.children:
                assert node in self._leaf_nodes, \
                    f"Leaf node missing from _leaf_nodes"
