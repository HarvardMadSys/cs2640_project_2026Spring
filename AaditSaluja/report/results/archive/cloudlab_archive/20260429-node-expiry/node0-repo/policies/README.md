...
```sh
./scripts/run_posix_bench.sh --policy predictive
./scripts/run_posix_bench.sh --policy LLM_policy
```

Or pass an explicit plugin file:

```sh
./scripts/run_posix_bench.sh --policy-file policies/example_predictive.py
```
