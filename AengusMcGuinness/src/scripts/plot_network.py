#!/usr/bin/env python3
"""Plot NIC bytes/op rows produced by the experiment runner scripts."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Plot server NIC bytes/op results.")
    parser.add_argument(
        "--csv",
        required=True,
        help="Input network CSV produced by runner scripts.",
    )
    parser.add_argument(
        "--outdir",
        default="plots/network",
    ...
    parser.add_argument(
        "--title",
        default="Server NIC Bytes Per Operation",
...
def main() -> int:
    ...
    total = aggregate(rows, args.x, args.group, "total_bytes_per_operation")
    rx = aggregate(rows, args.x, args.group, "rx_bytes_per_operation")
    tx = aggregate(rows, args.x, args.group, "tx_bytes_per_operation")

    save_plot(
        total,
        args.x,
        outdir,
        f"total_bytes_per_operation_vs_{args.x}.png",
        f"{args.title}: Total Bytes/Op vs {args.x}",
    ...
    save_plot(
        ...
        args.x,
        outdir,
        f"rx_bytes_per_operation_vs_{args.x}.png",
        f"{args.title}: RX Bytes/Op vs {args.x}",
        "RX NIC bytes / operation",
    )
    save_plot(
        tx,
        args.x,
        outdir,
        f"tx_bytes_per_operation_vs_{args.x}.png",
        f"{args.title}: TX Bytes/Op vs {args.x}",
        "TX NIC bytes / operation",
    )
    return 0
