...
from vllm.v1.core.kv_cache_utils import (KVCacheBlock, KVCacheBlocks,
                                         KVCacheBlockSegment)
...
class KVCacheManager:
    def __init__(
        ...
        self.coordinator = get_kv_cache_coordinator(
            ...
            block_manager_algo=block_manager_algo,
        )
    ...
    def get_computed_blocks(self, request: Request) -> list[KVCacheBlockSegment]:
        ...
        Returns:
            A list of tuple (cache segment) containing:
                - A list of blocks in this segment that are computed.
                - Start index.
                - Length.
        """
        ...
        if not self.enable_caching or request.skip_reading_prefix_cache:
            return [KVCacheBlockSegment(self.empty_kv_cache_blocks, 0, 0)]
        ...
        match_segements = self.coordinator.match_prefix_cache(
        ...
        if self.log_stats:
            ...
            self.prefix_cache_stats.record(
                ...
                num_hits=sum(seg.length_in_blocks for seg in match_segements),
                preempted=request.num_preemptions > 0,
            )

        return match_segements
    ...
    def allocate_slots(
        ...
        if (num_new_tokens == 0 and num_new_computed_tokens == 0
    ...
    def free(self, request: Request) -> None:
        ...
        """
        self.release_computed_segments(request)
    ...
    def get_block_ids(self, request_id: str) -> tuple[list[int], ...]:
        # Get the blocks for each KV cache group.
        return self.coordinator.get_blocks(request_id)

    def touch_computed_segments(self, request: Request) -> None:
        """Add a reference to all blocks in the request's computed segments.
        This pins them so they are not evicted while waiting to be allocated.
        """
        if not self.enable_caching:
            return
        for seg in request.computed_segments:
            for group_blocks in seg.blocks.blocks:
                self.block_pool.touch(group_blocks)

    def release_computed_segment(self, segment: KVCacheBlockSegment) -> None:
        """Release the reference to the blocks in the given computed segment."""
        if not self.enable_caching:
            return
        for group_blocks in segment.blocks.blocks:
            self.block_pool.free_blocks(group_blocks)

    def release_computed_segments(self, request: Request) -> None:
        """Release references to all remaining computed segments and clear them."""
        for seg in request.computed_segments:
            self.release_computed_segment(seg)
        request.computed_segments.clear()
