...
class SingleTypeKVCacheManager(ABC):
    ...
    def get_num_blocks_to_allocate(
        ...
        if request_id in self.num_cached_block:
            ...
            # If the request has new computed blocks, we need to fall through
            # to check if we can allocate more blocks.
    ...
    def allocate_new_computed_blocks(
        ...
        if request_id in self.num_cached_block and len(new_computed_blocks) == 0:
            # Fast-path: a running request won't have any new prefix-cache hits.
            # It should not have any new computed blocks.
            return

        # A new request or a running request with new computed blocks.
        ...
        if num_skipped_blocks > 0:
            ...
            # We need to account for the blocks fully skipped that are already in req_blocks
            num_req_blocks = len(req_blocks)
            num_new_skipped = max(0, num_skipped_blocks - num_req_blocks)
            new_computed_blocks = new_computed_blocks[num_new_skipped:]
            ...
            req_blocks.extend([self._null_block] * num_new_skipped)
    ...
    def free(self, request_id: str) -> None:
        ...
        self.block_pool.free_blocks(req_blocks, record_request=True)
    ...
    def match_prefix_cache(
        ...
        dcp_world_size: int = 1,
        pcp_world_size: int = 1,
    ) -> list[KVCacheBlockSegment]:
        """
        Get all the cache hit segments for the blocks. A cache hit segment is
        a contiguous segment of blocks that hits the cache. The segments should
        be a common prefix for all the kv cache groups in `kv_cache_group_ids`.
        If no cache hit is found, return an empty list.
        ...
        Returns:
            A list of cache hit segments for each kv cache group.
        """
...
class FullAttentionManager(SingleTypeKVCacheManager):
    ...
    def match_prefix_cache(
        ...
        segments: list[KVCacheBlockSegment] = []
        curr_segment_blocks: list[list[KVCacheBlock]] = [
            [] for _ in range(len(kv_cache_group_ids))
        ]
        curr_segment_start = -1

        for i in range(max_num_blocks):
            if cached_blocks := block_pool.get_cached_block(
                    block_hashes[i], kv_cache_group_ids):
                if curr_segment_start == -1:
                    curr_segment_start = i
                for curr_blocks, cached in zip(curr_segment_blocks,
                                               cached_blocks):
                    curr_blocks.append(cached)
            else:
                if curr_segment_start != -1:
                    # End current segment
                    segments.append(
                        KVCacheBlockSegment(
                            blocks=KVCacheBlocks(tuple(curr_segment_blocks)),
                            start_block_index=curr_segment_start,
                            length_in_blocks=len(curr_segment_blocks[0])))
                    curr_segment_start = -1
                    curr_segment_blocks = [
                        [] for _ in range(len(kv_cache_group_ids))
                    ]
        ...
        # Pruning for eagle and alignment
        # Pruning only makes sense for the last segment if it would otherwise
        # satisfy Eagle or alignment requirements at its end.
        last_seg = segments[-1]
        last_blocks_tuple = last_seg.blocks.blocks
        if use_eagle and last_blocks_tuple[0]:
            # Prune last block of last segment
            new_blocks = []
            for group_blocks in last_blocks_tuple:
                new_blocks.append(list(group_blocks)[:-1])
            last_seg.blocks = KVCacheBlocks(tuple(new_blocks))
            last_seg.length_in_blocks -= 1
            if last_seg.length_in_blocks == 0:
                segments.pop()

        if not segments:
            return []

        # Alignment pruning for the last segment
        last_seg = segments[-1]
        last_blocks_tuple = last_seg.blocks.blocks
        while (block_size != alignment_tokens and
               (last_seg.start_block_index + last_seg.length_in_blocks) *
               block_size % alignment_tokens != 0):
            new_blocks = []
            for group_blocks in last_blocks_tuple:
                new_blocks.append(list(group_blocks)[:-1])
            last_seg.blocks = KVCacheBlocks(tuple(new_blocks))
...
class SlidingWindowManager(SingleTypeKVCacheManager):
    ...
    def match_prefix_cache(
        ...
        blocks = cls.find_longest_cache_hit(
            block_hashes,
            max_length,
            kv_cache_group_ids,
            block_pool,
        ...
        from vllm.v1.core.kv_cache_utils import KVCacheBlocks
        segment = KVCacheBlockSegment(
            blocks=KVCacheBlocks(tuple(blocks)),
            start_block_index=0,
            length_in_blocks=len(blocks),
        )
        return [segment,]
