...
def legacy_get_computed_blocks(manager: KVCacheManager, request: Request, block_size: int):
    match_segments = manager.get_computed_blocks(request)
    if match_segments and match_segments[0].start_block_index == 0:
        return match_segments[0].blocks, match_segments[0].length_in_blocks * block_size
...
def test_prefill(hash_fn):
    ...
    computed_blocks, num_computed_tokens = legacy_get_computed_blocks(manager, req0, block_size)
    ...
    computed_blocks, num_computed_tokens = legacy_get_computed_blocks(manager, req1, block_size)
    ...
    free_block_manager = manager.block_pool.free_block_manager
    assert free_block_manager.num_free_blocks == 5

    manager.free(req0)
    manager.free(req1)

    # All blocks should be available.
    assert free_block_manager.num_free_blocks == 10
    # The order should be
    ...
    # [common (3, 2, 1)]
    assert [
        b.block_id for b in manager.block_pool.free_block_manager.get_all_free_blocks()
    ...
    computed_blocks, num_computed_tokens = legacy_get_computed_blocks(manager, req2, block_size)
    ...
    assert free_block_manager.num_free_blocks == 6
    assert all([b.ref_cnt == 0 for b in free_block_manager.get_all_free_blocks()])
    assert len([b for b in free_block_manager.get_all_free_blocks()]) == 6

    manager.free(req2)

    # Cache miss and eviction.
    req3 = make_request("3", [99] * (16 * 10), block_size, hash_fn)
    computed_blocks, num_computed_tokens = legacy_get_computed_blocks(manager, req3, block_size)
    assert not computed_blocks.blocks[0]
    assert num_computed_tokens == 0
    blocks = manager.allocate_slots(
    ...
    assert free_block_manager.num_free_blocks == 0
    assert (
        free_block_manager.fake_free_list_head.next_free_block
        is free_block_manager.fake_free_list_tail
    )
    assert (
        free_block_manager.fake_free_list_tail.prev_free_block
        is free_block_manager.fake_free_list_head
    )
...
def _test_partial_request_hit(
    ...
    computed_blocks, num_computed_tokens = legacy_get_computed_blocks(manager, req, block_size)
...
def test_prefill_plp():
    ...
    # At this point, we should have 5 free blocks left.
    assert manager.block_pool.free_block_manager.num_free_blocks == 5

    manager.free(req0)
    manager.free(req1)

    # All blocks should be available.
    assert manager.block_pool.free_block_manager.num_free_blocks == 10
    # The order should be
    # [unallocated (6, 7, 8, 9, 10)]
    # [unique_req0 (4)]
    # [unique_req1 (5)]
...
def test_evict():
    ...
    # 10 - (6 + 3) == 1
    assert manager.block_pool.free_block_manager.num_free_blocks == 1
    ...
    assert manager.block_pool.free_block_manager.num_free_blocks == 7
...
def test_reset_prefix_cache():
    ...
    computed_blocks, _ = legacy_get_computed_blocks(manager, req1, block_size)
    assert len(req1.block_hashes) == 3
    assert len(computed_blocks.blocks[0]) == 3
    blocks = manager.allocate_slots(
...
def test_eagle_enabled_removes_last_block():
    ...
    # Prime the cache
    computed_blocks, _ = legacy_get_computed_blocks(manager, req, block_size)
    manager.allocate_slots(
    ...
    computed_blocks, num_tokens = legacy_get_computed_blocks(manager, req_eagle, block_size)
...
def test_eagle_with_sliding_window():
    ...
    computed_blocks, num_tokens = legacy_get_computed_blocks(manager, req_after_evict, block_size)
