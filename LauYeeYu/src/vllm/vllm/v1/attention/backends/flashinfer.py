...
class FlashInferMetadata:
    ...
    # --- One-Step Hole Filling ---

    use_hole_fill: bool = False
    """If True, some requests have non-contiguous gaps filled in one step."""

    hole_fill_prefix_wrapper: (
        "BatchPrefillWithPagedKVCacheWrapper | None") = None
    """Wrapper for shared-prefix attention (cached blocks before first gap)."""

    hole_fill_suffix_wrapper: (
        "BatchPrefillWithPagedKVCacheWrapper | None") = None
    """Wrapper for per-gap suffix attention."""

    hole_fill_token_ranges: list[tuple[int, int]] | None = None
    """For each hole-fill sub-request: (start_offset, length) in the
    flat token array, used to slice outputs for merge_state."""

    hole_fill_num_shared_prefix_tokens: int = 0
    """Number of shared prefix tokens (for non-causal prefix attention)."""

    # Indices of hole-fill tokens within the prefill portion.
    hole_fill_output_slice: tuple[int, int] | None = None
    """(start, end) slice in the output tensor for hole-fill tokens."""
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def _get_hole_fill_prefix_wrapper(self):
        """Get a prefill wrapper for hole-fill shared prefix attention."""
        if not hasattr(self, '_hole_fill_prefix_wrapper') or \
                self._hole_fill_prefix_wrapper is None:
            self._hole_fill_prefix_wrapper = \
                BatchPrefillWithPagedKVCacheWrapper(
                    self._get_workspace_buffer(),
                    get_kv_cache_layout())
        return self._hole_fill_prefix_wrapper

    def _get_hole_fill_suffix_wrapper(self):
        """Get a prefill wrapper for hole-fill per-gap suffix attention."""
        if not hasattr(self, '_hole_fill_suffix_wrapper') or \
                self._hole_fill_suffix_wrapper is None:
            self._hole_fill_suffix_wrapper = \
                BatchPrefillWithPagedKVCacheWrapper(
                    self._get_workspace_buffer(),
                    get_kv_cache_layout())
        return self._hole_fill_suffix_wrapper
    ...
    def build(
        ...
        # Step 2.5: Handle one-step hole filling with cascade attention.
        # For hole-filling requests, we decompose attention into:
        #   1. Shared prefix attention (cached blocks before first gap)
        #   2. Per-gap suffix attention (blocks from first gap start onward)
        # Then merge with merge_state for each gap.
        hole_fill_gap_ranges = common_attn_metadata.hole_fill_gap_ranges
        if hole_fill_gap_ranges and not use_cascade:
            assert seq_lens_np is not None
            assert num_blocks_np is not None
            attn_metadata.use_hole_fill = True

            # Build sub-request metadata for prefix and suffix wrappers.
            # Prefix: all gap tokens attend to shared prefix blocks.
            # Suffix: each gap attends to blocks from prefix_end onward.
            prefix_qo_indptr = [0]
            prefix_kv_page_indptr = [0]
            prefix_kv_page_indices_list: list[int] = []
            prefix_kv_last_page_len_list: list[int] = []

            suffix_qo_indptr = [0]
            suffix_kv_page_indptr = [0]
            suffix_kv_page_indices_list: list[int] = []
            suffix_kv_last_page_len_list: list[int] = []

            hole_fill_token_ranges: list[tuple[int, int]] = []
            total_hole_fill_tokens = 0

            for req_idx, gap_ranges in hole_fill_gap_ranges.items():
                # The shared prefix is all blocks before the first gap.
                first_gap_start = gap_ranges[0][0]
                num_prefix_blocks = first_gap_start // page_size
                req_block_table = block_table_tensor[req_idx]

                # Compute the token offset for this request in the
                # flattened output.
                req_token_start = int(
                    qo_indptr_cpu[req_idx].item())

                gap_offset = 0
                for gap_start, gap_len in gap_ranges:
                    # Record token range for merge_state.
                    hole_fill_token_ranges.append(
                        (req_token_start + gap_offset, gap_len))
                    total_hole_fill_tokens += gap_len

                    # --- Prefix sub-request ---
                    # Gap tokens attend to shared prefix (non-causal).
                    prefix_qo_indptr.append(
                        prefix_qo_indptr[-1] + gap_len)
                    prefix_kv_page_indptr.append(
                        prefix_kv_page_indptr[-1] + num_prefix_blocks)
                    for b in range(num_prefix_blocks):
                        prefix_kv_page_indices_list.append(
                            int(req_block_table[b].item()))
                    if num_prefix_blocks > 0:
                        prefix_last = first_gap_start % page_size
                        prefix_kv_last_page_len_list.append(
                            page_size if prefix_last == 0 else prefix_last)
                    else:
                        prefix_kv_last_page_len_list.append(0)

                    # --- Suffix sub-request ---
                    # Gap tokens attend to blocks from prefix_end to gap_end
                    # (causal). This includes earlier gap blocks and cached
                    # segment blocks between gaps.
                    gap_end = gap_start + gap_len
                    gap_end_block = cdiv(gap_end, page_size)
                    suffix_start_block = num_prefix_blocks
                    num_suffix_blocks = gap_end_block - suffix_start_block

                    suffix_qo_indptr.append(
                        suffix_qo_indptr[-1] + gap_len)
                    suffix_kv_page_indptr.append(
                        suffix_kv_page_indptr[-1] + num_suffix_blocks)
                    for b in range(suffix_start_block, gap_end_block):
                        suffix_kv_page_indices_list.append(
                            int(req_block_table[b].item()))

                    suffix_last = gap_end % page_size
                    suffix_kv_last_page_len_list.append(
                        page_size if suffix_last == 0 else suffix_last)

                    gap_offset += gap_len

            attn_metadata.hole_fill_token_ranges = hole_fill_token_ranges
            attn_metadata.hole_fill_num_shared_prefix_tokens = (
                first_gap_start if hole_fill_gap_ranges else 0)

            # Plan prefix wrapper (non-causal: all KV positions < Q positions).
            if num_prefix_blocks > 0:
                prefix_wrapper = self._get_hole_fill_prefix_wrapper()
                prefix_wrapper.plan(
                    torch.tensor(prefix_qo_indptr,
                                 dtype=torch.int32, device="cpu"),
                    torch.tensor(prefix_kv_page_indptr,
                                 dtype=torch.int32, device="cpu"),
                    torch.tensor(prefix_kv_page_indices_list,
                                 dtype=torch.int32, device="cpu"),
                    torch.tensor(prefix_kv_last_page_len_list,
                                 dtype=torch.int32, device="cpu"),
                    self.num_qo_heads,
                    self.num_kv_heads,
                    self.head_dim,
                ...
                attn_metadata.hole_fill_prefix_wrapper = prefix_wrapper

            # Plan suffix wrapper (causal).
            suffix_wrapper = self._get_hole_fill_suffix_wrapper()
            suffix_wrapper.plan(
                torch.tensor(suffix_qo_indptr,
                             dtype=torch.int32, device="cpu"),
                torch.tensor(suffix_kv_page_indptr,
                             dtype=torch.int32, device="cpu"),
                torch.tensor(suffix_kv_page_indices_list,
                             dtype=torch.int32, device="cpu"),
                torch.tensor(suffix_kv_last_page_len_list,
                             dtype=torch.int32, device="cpu"),
                self.num_qo_heads,
                self.num_kv_heads,
                self.head_dim,
            ...
            attn_metadata.hole_fill_suffix_wrapper = suffix_wrapper

            # Compute the output slice for hole-fill tokens.
            if hole_fill_token_ranges:
                first_start = hole_fill_token_ranges[0][0]
                last_start, last_len = hole_fill_token_ranges[-1]
                attn_metadata.hole_fill_output_slice = (
                    first_start, last_start + last_len)

        # When using hole fill, hole-fill requests are in the prefill
        # section but must NOT be processed by the regular prefill wrapper
        # (it would apply incorrect causal masking). Adjust counts.
        # Hole-fill requests are at the end of the batch (longest prefills).
        num_hole_fill_reqs = 0
        if attn_metadata.use_hole_fill and hole_fill_gap_ranges:
            num_hole_fill_reqs = len(hole_fill_gap_ranges)
            hole_fill_tokens = sum(
                sum(gl for _, gl in gr)
                for gr in hole_fill_gap_ranges.values())
            num_prefills -= num_hole_fill_reqs
            num_prefill_tokens -= hole_fill_tokens
            attn_metadata.num_prefills = num_prefills
            attn_metadata.num_prefill_tokens = num_prefill_tokens

        # Effective number of requests for regular prefill/decode planning
        # (excludes hole-fill requests at the end).
        num_regular_reqs = num_reqs - num_hole_fill_reqs

        # Step 3: Handle prefill and decode pathways case by case
        ## PREFILL PATHWAY
        if num_prefills > 0:
            # Slices for shared prefill metadata.
            # When hole-fill requests are present, they are at the end
            # of the batch and excluded from regular prefill planning.
            prefill_start = num_decodes
            prefill_end = num_regular_reqs
            qo_indptr_prefill_cpu = (
                qo_indptr_cpu[prefill_start:prefill_end + 1]
            ...
            if prefill_use_trtllm:
                ...
                attn_metadata.prefill = TRTLLMPrefill(
                    ...
                    seq_lens=seq_lens[prefill_start:prefill_end],
                ...
                # Slicing CPU buffers that are only needed for FI native
                # prefills. Use prefill_end to exclude hole-fill requests.
                paged_kv_last_page_len_prefill_cpu = (
                    self.paged_kv_last_page_len.cpu[
                        prefill_start:prefill_end])
                assert paged_kv_last_page_len_prefill_cpu.shape[0] == (
                    num_prefills)
                paged_kv_indptr_prefill_cpu = self.paged_kv_indptr.cpu[
                    prefill_start : prefill_end + 1
                ]
                assert paged_kv_indptr_prefill_cpu.shape[0] == (
                    num_prefills + 1)
                if self.use_dcp:
...
class FlashInferImpl(AttentionImpl):
    ...
    def forward(
        ...
        # One-step hole filling with cascade attention (merge_state).
        if attn_metadata.use_hole_fill:
            assert attn_metadata.hole_fill_suffix_wrapper is not None
            assert attn_metadata.hole_fill_token_ranges is not None

            # Collect hole-fill query tokens from each gap.
            hole_fill_queries = []
            for token_start, token_len in attn_metadata.hole_fill_token_ranges:
                hole_fill_queries.append(
                    query[token_start:token_start + token_len])
            hf_query = torch.cat(hole_fill_queries, dim=0)
            hf_num_tokens = hf_query.shape[0]

            if attn_metadata.hole_fill_prefix_wrapper is not None:
                # Phase 1: Shared prefix attention (non-causal).
                # Returns output [N, H, D] and lse [N, H].
                prefix_output, prefix_lse = (
                    attn_metadata.hole_fill_prefix_wrapper.run(
                        hf_query,
                        kv_cache_permute,
                        k_scale=layer._k_scale_float,
                        v_scale=layer._v_scale_float,
                ...
                # Phase 2: Per-gap suffix attention (causal).
                suffix_output, suffix_lse = (
                    attn_metadata.hole_fill_suffix_wrapper.run(
                        hf_query,
                        kv_cache_permute,
                        k_scale=layer._k_scale_float,
                        v_scale=layer._v_scale_float,
                ...
                # Phase 3: Merge prefix and suffix with merge_state.
                # FlashInfer returns LSE as [N, H] in log base 2.
                # merge_attn_states expects [H, N] in natural log.
                LN2 = 0.6931471805599453
                prefix_lse = (prefix_lse * LN2).T.contiguous()
                suffix_lse = (suffix_lse * LN2).T.contiguous()
                hf_output = torch.empty_like(suffix_output)
                merge_attn_states(
                    hf_output,
                    prefix_output, prefix_lse,
                    suffix_output, suffix_lse,
                ...
                # No shared prefix (first gap starts at position 0).
                # Just run suffix attention directly.
                hf_output = (
                    attn_metadata.hole_fill_suffix_wrapper.run(
                        hf_query,
                        kv_cache_permute,
            ...
            # Write hole-fill outputs back to the output tensor.
            offset = 0
            for token_start, token_len in attn_metadata.hole_fill_token_ranges:
                output[token_start:token_start + token_len] = (
                    hf_output[offset:offset + token_len])
                offset += token_len

            # Process remaining non-hole-fill tokens (decodes and normal
            # prefills) through the regular pathway below. If all tokens
            # are hole-fill, we can return early.
            if hf_num_tokens == num_actual_tokens:
        ...
        # When hole-fill is active, hole-fill tokens are excluded from
        # num_prefill_tokens and handled separately above.
